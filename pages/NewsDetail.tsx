...
const NewsDetail: React.FC = () => {
  ...
  const handleComment = (e: React.FormEvent) => {
    ...
    setAuthorEmail('');
    setUserScore(10);
    alert("Review transmitted to the grid.");
  };

  const visibleComments = (news.comments || []).filter(c => c.isVisible);
    ...
    <div className={`min-h-screen pb-20 transition-colors duration-500 animate-fadeIn ${theme === 'dark' ? 'bg-black text-white' : 'bg-slate-50 text-slate-900'}`}>
      <div className="w-full max-w-7xl mx-auto md:px-8">
        ...
        <main className="max-w-5xl mx-auto py-10 px-6">
          <div className="relative aspect-video rounded-[3rem] overflow-hidden mb-12 shadow-2xl border border-slate-200 dark:border-white/10 group">
            ...
            <div className="absolute bottom-0 p-10 lg:p-16">
              ...
              <h1 className="text-3xl md:text-6xl font-black leading-[0.9] tracking-tighter text-white uppercase italic drop-shadow-2xl mb-4">{news.title}</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              {news.videoUrl && (
                <section className="space-y-6 animate-fadeIn">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Multiversal Stream Active</span>
                  </div>
                  <div className="aspect-video rounded-[3rem] overflow-hidden bg-black border border-white/10 shadow-[0_20px_60px_rgba(242,13,13,0.15)] ring-1 ring-white/5">
                    {ytId ? (
                      <iframe
              ...
              <section className="pt-20 border-t border-white/5 space-y-12">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                  ...
                  <div className="p-6 bg-white/5 border border-white/10 rounded-[2rem] flex items-center gap-4">
                    <span className="text-5xl font-black italic text-primary-blue">{averageScore}</span>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Average Grid Score</span>
                      <span className="text-[9px] font-bold text-gray-600 uppercase">{(news.userRatings || []).length} Transmissions</span>
                    </div>
                  </div>
                </div>

                <div className={`p-10 border rounded-[3.5rem] shadow-2xl transition-colors ${theme === 'dark' ? 'bg-surface-dark border-white/5' : 'bg-white border-slate-100'}`}>
                  <form onSubmit={handleComment} className="space-y-8">
                    ...
                    <div className="space-y-3">
                      ...
                      <textarea required value={commentText} onChange={e => setCommentText(e.target.value)} rows={4} placeholder="Establishing connection... Write your review." className={`w-full border rounded-[2rem] p-8 text-sm font-medium outline-none focus:border-primary-blue transition-all resize-none ${theme === 'dark' ? 'bg-black/40 border-white/10 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}></textarea>
                    </div>

                    <button type="submit" className="w-full py-5 bg-primary-blue text-black font-black text-[11px] uppercase tracking-[0.5em] rounded-[2rem] shadow-[0_10px_40px_rgba(0,242,255,0.2)] hover:scale-[1.02] active:scale-95 transition-all">Broadcast Review</button>
                  </form>
                </div>

                <div className="space-y-8">
                  {visibleComments.map(comment => (
                    <div key={comment.id} className={`p-8 border rounded-[3rem] flex flex-col md:flex-row gap-8 group transition-colors ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
                      <div className="shrink-0 flex flex-col items-center">
                        <img src={comment.avatar} className="size-16 rounded-[1.5rem] border-2 border-primary-blue/20 mb-4" alt="" />
                        <div className="size-12 rounded-xl bg-primary-blue/10 border border-primary-blue/20 flex items-center justify-center">
                          <span className="text-sm font-black text-primary-blue italic leading-none">{comment.userScore}</span>
                          <span className="text-[7px] font-black uppercase text-primary-blue/60">Pts</span>
                        </div>
                      </div>
                      <div className="flex-1 space-y-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className={`text-lg font-black uppercase italic tracking-tighter ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{comment.author}</h4>
                            <span className="text-[10px] font-bold text-gray-500 uppercase">{comment.date}</span>
                          </div>
                        </div>
                        <p className={`italic leading-relaxed ${theme === 'dark' ? 'text-gray-400' : 'text-slate-600'}`}>"{comment.text}"</p>
                      </div>
                    </div>
                  ))}
            ...
            <aside className="lg:col-span-4 space-y-12">
              <div className={`p-8 border rounded-[3rem] sticky top-32 transition-colors ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
                <h3 className="text-xl font-black italic uppercase tracking-tighter mb-8 flex items-center gap-3">
                  <span className="size-2 bg-primary-blue rounded-full animate-pulse"></span> Related <span className="text-primary-blue">Signals</span>
                </h3>
                <div className="space-y-6">
                  {relatedContent.map(item => (
                    <Link key={item.id} to={`/news/${item.id}`} className={`group flex gap-4 items-center p-4 rounded-2xl transition-all border border-transparent ${theme === 'dark' ? 'hover:bg-white/5 hover:border-white/5' : 'hover:bg-slate-50 hover:border-slate-100'}`}>
                      <img src={item.thumbnailUrl || item.imageUrl} className="size-20 rounded-2xl object-cover shadow-lg group-hover:scale-105 transition-transform" alt="" />
                      <div className="flex-1 min-w-0">
                        <div className="flex gap-2 mb-1">
                          {item.categories.slice(0, 1).map(c => (
                            <span key={c} className="text-[8px] font-black uppercase text-primary-blue tracking-widest">{c}</span>
                          ))}
                        </div>
                        <h4 className={`text-xs font-black uppercase italic leading-tight group-hover:text-primary-blue transition-colors truncate ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{item.title}</h4>
                        <p className="text-[9px] font-bold text-gray-500 uppercase mt-1">{item.type} • {new Date(item.date).toLocaleDateString()}</p>
                      </div>
                    </Link>
                  ))}
