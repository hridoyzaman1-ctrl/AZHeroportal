...
const AdminRankings: React.FC = () => {
  ...
  const [lists, setLists] = useState<RankingList[]>(storageService.getRankingLists());
  ...
  const handleSave = () => {
    ...
    setLists(updated);
    storageService.saveRankingLists(updated);
    setEditingList(null);
  };
    ...
    <div className="flex h-screen bg-[#050505] text-white overflow-hidden">
       <aside className="w-72 bg-[#0c0c0c] border-r border-white/5 flex flex-col shrink-0">
        ...
        <nav className="flex-1 p-8 space-y-4">
          ...
          <Link to="/admin/rankings" className="flex items-center gap-5 p-5 rounded-2xl bg-primary-blue/10 text-primary-blue border border-primary-blue/20"><span className="material-symbols-outlined">trophy</span><span className="font-bold text-[11px] uppercase tracking-widest">Ranks</span></Link>
        </nav>
      </aside>
