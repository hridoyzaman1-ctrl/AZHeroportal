...
// Fix: Added Link import from react-router-dom
...
const Rankings: React.FC = () => {
  ...
  const lists = storageService.getRankingLists();
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-10">
        ...
        {rankedItems.length > 0 ? (
          <div className="space-y-12">
            {rankedItems.map((item: any, idx) => (
               <div key={item.id} className={`group grid grid-cols-1 lg:grid-cols-12 gap-10 p-10 rounded-[3rem] border transition-all ${theme === 'dark' ? 'bg-surface-dark/50 border-white/5' : 'bg-white border-slate-200'}`}>
                  ...
                  <div className="lg:col-span-7 flex flex-col justify-center space-y-6">
                    ...
                    {/* Fix: Link component from react-router-dom */}
