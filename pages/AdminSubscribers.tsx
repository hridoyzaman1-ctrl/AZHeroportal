  ...
  return (
    <AdminLayout title="SIGNAL SUBSCRIBERS" subtitle="Newsletter Nodes">
      <div className="flex-1 flex flex-col min-w-0 bg-[#0a0f1a] h-full">
        <div className="px-4 md:px-12 py-6 md:py-10 border-b border-white/5 flex flex-col sm:row justify-end gap-4">
          ...
          <div className="bg-white/5 border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
            <table className="w-full text-left">
              <thead className="bg-white/5 text-[9px] font-black uppercase tracking-[0.2em] text-gray-500 border-b border-white/5">
                <tr>
                  <th className="px-6 md:px-10 py-6">Identity (Email)</th>
                  <th className="px-6 md:px-10 py-6 hidden sm:table-cell">Linked Date</th>
                  <th className="px-6 md:px-10 py-6 text-right">Ops</th>
                </tr>
              </thead>
                ...
                {filtered.map(sub => (
                  <tr key={sub.id} className="hover:bg-white/5 transition-all">
                    <td className="px-6 md:px-10 py-6 md:py-8">
                      <p className="text-sm md:text-base font-black text-white truncate max-w-[200px] sm:max-w-none">{sub.email}</p>
                      <span className="sm:hidden text-[7px] font-bold text-gray-600 uppercase tracking-widest">{sub.date}</span>
                    </td>
